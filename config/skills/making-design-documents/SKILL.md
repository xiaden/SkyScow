...
**Purpose:** Produce language-idiomatic design documents (DDs) for features. Load this before writing a design doc or evaluating architectural trade-offs. For decomposing a DD into a Change DAG, use `decomposing-design-documents`.
...
| **2. Body** | This SKILL.md (core workflow, language selection, anti-patterns) | When the agent decides the task involves feature design or decomposition |
| **3. References** | Language-specific design patterns, architectural principles & trade-off analysis, decomposition methodology | Only when the agent reads the relevant reference for the target language, design principle, or Change DAG authoring task |

### What Goes Where

| Keep in SKILL.md (always loaded) | Move to references/ (loaded on demand) |
|----------------------------------|----------------------------------------|
| Design workflow overview | Full Change DAG decomposition methodology (`planning.md`) |
...
| Pointers to `references/` | Change DAG authoring instructions |
...
- Creating a Change DAG from requirements
...
- You already have a validated Change DAG and just need to execute it mechanically
- The task is pure bug-fixing with no design component
- You need to decompose a design document into a Change DAG — use `decomposing-design-documents` instead
...
For decomposing a design document into a Change DAG with semantic decomposition and exact work, use `decomposing-design-documents`. The design skill's job stops at producing the design document.
...
| **Analysis paralysis** | Perfect design doc, zero code shipped. The best decompositions enable incremental implementation and are refined as you learn. |
| **Over-decomposition** | Splitting a feature into 20 DAG nodes when 4 would do. Each extra semantic layer adds overhead. |
...
| [`references/planning.md`](file:///home/opencode/.config/opencode/skills/making-design-documents/references/planning.md) | Full Change DAG decomposition methodology, node/edge model, best practices |
