...
|---|---|
| An accepted DD may leave a changed behavior path inconsistent | `impact_closure` (default): will this specific accepted change leave a known behavior path partially changed or inconsistent? |
| An accepted DD explicitly establishes bounded migration intent | `migration_scan`: where should that accepted migration propagate? |
...
Do not compare a DD to the verbatim request or immutable ledger, emit `REQUIREMENT_DRIFT`, validate `request_context.path`, inspect lifecycle/supersession or stale artifacts, prescribe mock/real tests, resolve unresolved callgraph edges, validate generalized ownership closure, create migration phases, or amend Change DAGs. RnD-Manager owns independent CTX/ledger/final-DD conformance; Change-DAG-Author owns DAG assignment and amendment; artifact/design owners retain lifecycle.

Findings route to an owner for disposition. They do not become requirements, contracts, ADRs, migration scope, or implementation obligations automatically.
