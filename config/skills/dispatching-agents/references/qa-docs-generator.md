...
| `reason` | Why docs are needed | "Public API endpoint — no documentation for DELETE behavior" |
  | `slug` | Change DAG identity for the durable record | `api-change` |
...
The record requires `writer`/`agent` = `qa-docs-generator`, the Change DAG slug (or the task family for standalone publication records) and positive round, a stable
