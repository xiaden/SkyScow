...
Dispatch qa-repo-reviewer-journey as one of the read-only whole-tree reviewers
...
- You need candidate/diff/push-gate review — use `qa-reviewer-journey`.
...
```text
Review the complete resolved tree for end-to-end journey correctness.
...
clean review (including when all material journeys complete correctly), otherwise
a single object mapping stable finding IDs to transport findings with all
required fields (`severity`, `finding_class`, `scope_basis`, `files`, `location`,
`trigger`, `problem_description`, `evidence`, `repair_route`,
`recommended_action`, `expected_behavior`). Optional `journey` and `break_point`
add evidence but never replace a required field. No prose, no PASS/FAIL heading,
no Markdown fences.
```
