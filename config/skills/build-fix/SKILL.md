...
5. **Verify the build passes** — re-run the repository's build command after all fixes, then run the repository's test command for the changed surface when one exists; do not assume a test suite such as `npm test` exists (see `/home/opencode/.config/opencode/instructions/validation-mandate.md`)
