...
The contracts ledger (`CONTRACTS.md`) is a living document updated after every validated plan. Exec-Planner subagents receive its content as context.
...
**Design doc:** `artifacts/designs/pending/{feature}/DD.md`
...
{Rules extracted from project architecture documentation relevant to this feature. Examples:}

- Workflows take `db: Database`, never services
- Persistence uses `DatabaseLike` from `src.persistence.database_client`
...
| --- | --- | --- | --- |
| TypedDictName | `src.helpers.dto.xxx_dto` | field1, field2, ... | {letter} |
