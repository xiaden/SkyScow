...
Review degraded and partial states against the current plan's bounded responsibilities. For every incomplete finding, classify `ownership` as `CURRENT_PLAN`, `DOWNSTREAM_PLAN`, or `PLANNING_GAP`. `DOWNSTREAM_PLAN` requires a present, schema-valid, non-superseded later plan in the same ordered plan set; include `downstreamPlan` and set `blocks_push: false`. Current-plan work and unowned gaps remain normal blocking findings. Do not infer ownership from likely future work or informal annotations.
