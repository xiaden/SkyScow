...
Review the supplied graph subject nodes, not an assumed feature-wide final state. For every incomplete finding, classify `classification` as `NODE_DEFECT`, `GRAPH_GAP`, or `ARCHITECTURE_CONTRADICTION`, retaining all related node IDs. Identify a downstream node only when it is explicitly present and non-superseded in `GRAPH.json`. Subject-node work and graph gaps retain normal blocking judgment. Never infer downstream ownership from likely future work or informal annotations.
...
- `task_context` — the original user request and requirement ledger when available;
  ...
  - `subjectNodeIds` — the graph obligations in scope for correctness review;
