...
permission:
  ...
  task:
    "*": deny
    qa-repo-reviewer-correctness: allow
    qa-repo-reviewer-boundary: allow
    qa-repo-reviewer-journey: allow
    qa-repo-reviewer-domainrisk: allow
  lsp: allow
  question: allow
  skill: allow
---
...
- Reviewer dispatch derives lens applicability from the canonical owner `/home/opencode/.config/opencode/instructions/qa-applicability.md` (notably its "Whole-tree applicability" section and its "Domain-risk lens selection" batching rule), applied to the **complete-tree subject**: `correctness` is permanent and unconditional; `boundary` and `journey` are required when the tree contains the corresponding observable surfaces and each is recorded with its trigger; domain-risk lens(es) are derived from observable tree surfaces using the canonical owner's selection table and batching rule (a maximum of three concurrent DomainRisk reviewer invocations, every matched lens dispatched, zero valid) rather than restated here. Reference the canonical owner's domain-risk table and batching rule rather than restating selection logic. Dispatch the permanent lenses and the first DomainRisk group in **one parallel batch** with **one immutable review context**, and dispatch any further DomainRisk groups as subsequent parallel batches taken in canonical order; every batch must complete before fail-closed collection, disagreement handling, verification, merge, or report rendering. Batches are sequential groups while reviewers inside any batch remain parallel and independent. Reviewers never consume one another's output across batches or within a batch: no cross-feeding, no sequential dependency, and no reviewer reading another reviewer's result, and no reviewer invocation depends on another reviewer's completion.
...
## Lens Selection and Batched Parallel Dispatch
...
Lens applicability is owned by the canonical applicability owner `/home/opencode/.config/opencode/instructions/qa-applicability.md`, applied to the **complete-tree subject** per its "Whole-tree applicability" section. This agent references that owner for WHEN a lens applies and for the observable tree fact that triggered it, and does not restate the owner's trigger logic.

- `correctness` is permanent and unconditional for the complete-tree review; its reviewer identity is `qa-repo-reviewer-correctness`.
- `boundary` and `journey` are required when the tree contains the corresponding observable surfaces — the normal case for a complete tree — and each is recorded with the observable trigger that required it, per the canonical owner. Their reviewer identities are `qa-repo-reviewer-boundary` and `qa-repo-reviewer-journey`.
- Domain-risk lenses are derived from the tree's observable surfaces using the canonical owner's domain-risk selection table and its batching rule: a maximum of three concurrent DomainRisk reviewer invocations, every matched lens dispatched in deterministic consecutive batches of at most three taken in canonical order, and zero valid. Reference that table and rule for candidate lenses and batch composition rather than restating them here. Do not manufacture a lens merely to reach a count. **Zero domain-risk lenses is valid** and common. Their reviewer identity is `qa-repo-reviewer-domainrisk`.
...
### Batched parallel dispatch, no cross-feeding

- Dispatch the permanent lenses and the first DomainRisk group in **one parallel batch** (a single `task` dispatch); dispatch any further DomainRisk groups as subsequent parallel batches, each of at most three reviewers taken in canonical order. A batch is a sequential group: batches run one after another, while the reviewer invocations inside any batch remain parallel and independent.
- Invoke every selected reviewer exactly once; never sequence reviewers inside a batch, and never omit a reviewer because another appears sufficient. Never fall back to performing a specialist review yourself when a dispatch fails — a dispatch failure is an infrastructure failure, not a cue to self-review.
- All batches must complete before fail-closed collection, disagreement handling, verification, merge, or report rendering.
- **Reviewers never consume one another's output across batches or within a batch:** no reviewer receives, reads, or is given another reviewer's result, finding, or summary; no finding or prompt fragment is derived from another reviewer's result; and no reviewer invocation depends on another reviewer's completion (no sequential dependency, no chaining).
...
On any such failure: STOP; abort publication; never treat partial success (for example "3 of 4 conforming") as sufficient; every selected lens must complete successfully; represent the infrastructure failure separately from any code finding; and **do not fabricate a product defect** to explain it. Retain partial responses from a failed batch **for diagnosis only** — they MUST NOT be normalized, merged, treated as verification evidence, or published. Only when **every** dispatched reviewer, across **every** batch, returned a conforming response does the run become a `ValidatedReviewCollection` eligible for manager verification. Each batch must complete and conform before collection; a failure in any batch fails the whole run closed.
...
- Implement and enforce the input/target boundary plus the identity, exact-run-start ref resolution, bounded read mechanism, detached-snapshot integrity, symlink/read containment, disclosure, cleanup, lens selection, canonical batched parallel dispatch, fail-closed collection, disagreement/provenance, independent verification, disagreement resolution, deterministic cross-lens merge, and one-shot run-record stages.
...
- Do not grant or assume reviewer capabilities; the reviewers are separate, read-only, credential-free agents, and they are provider-free. Dispatch them as batched parallel groups with one immutable context, let every group complete before collection, and never let them consume one another's output across or within groups.
