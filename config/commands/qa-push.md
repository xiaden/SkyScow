...
5. Only when deterministic validation passes and the snapshot invariants still hold, dispatch in canonical batches the specialist lenses selected by the canonical applicability owner, `/home/opencode/.config/opencode/instructions/qa-applicability.md`; the canonical batching rule applies (zero matched lenses dispatch none; one to three dispatch in one parallel batch; more than three dispatch in consecutive batches of at most three in canonical order until every matched lens completes). That file is the single owner of which lenses apply and of the observable fact that triggered each; do not restate its trigger or selection logic here. Correctness remains the mandatory baseline, and each domain-risk invocation receives its own `assigned_lens` and is confined to it. Every reviewer receives the same immutable candidate context, deterministic results, and the isolated snapshot path. Reviewers are read-only and never see or consume each other's findings.
